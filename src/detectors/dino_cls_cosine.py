...
class DINOv2ClsPatchCosineDetector(BaseAnomalyDetector):
    ...
    def _extract_raw_scores(self, image: np.ndarray) -> tuple[np.ndarray, tuple[int, int]]:
        ...
        with torch.inference_mode():
            ...
            batch = tensor.unsqueeze(0).to(self._model.device)
            layer_outputs = self._model.model.get_intermediate_layers(
                batch,
                n=self._num_layers,
                return_class_token=True,
                norm=True,
            )
            patch_tokens, cls_token = layer_outputs[self._layer_idx]
